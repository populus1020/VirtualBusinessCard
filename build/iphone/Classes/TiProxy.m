...
//Rare exceptions to indicate a bug in the _virtualbusinesscard code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_virtualbusinesscard = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_virtualbusinesscard]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.virtualbusinesscard.module"
