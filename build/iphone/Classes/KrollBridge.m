...
NSString * VirtualBusinessCard$ModuleRequireFormat = @"(function(exports){"
...
@implementation VirtualBusinessCardObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_virtualbusinesscard);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create VirtualBusinessCard global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "VirtualBusinessCard" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_virtualbusinesscard = [[VirtualBusinessCardObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_virtualbusinesscard];
	
	NSString *_virtualbusinesscardNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _virtualbusinesscardNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_virtualbusinesscard valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:VirtualBusinessCard$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.virtualbusinesscard.kroll" 
